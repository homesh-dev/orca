...
export function isNewTurnEvent(source: AgentHookSource, eventName: unknown): boolean {
  ...
  switch (source) {
    ...
    case 'musecode':
      // Why: MuseCode emits Claude-compatible hook events (verified against muse 1.0.3), so UserPromptSubmit is its new-turn boundary too.
      return eventName === 'UserPromptSubmit'
    case 'codex':
      return eventName === 'SessionStart' || eventName === 'UserPromptSubmit'
    case 'gemini':
...
export function extractToolFields(
  ...
  switch (source) {
    ...
    // falls through
    case 'kimi':
    // Why: MuseCode hook payloads are Claude-shaped (verified against muse 1.0.3).
    // falls through
    case 'musecode':
